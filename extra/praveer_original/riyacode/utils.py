...
class Race_CNN(nn.Module): 
    def __init__(self):
        ...
        self.resnet18 = resnet18(pretrained=True) # resnet18 variable, this variable is linked to a class available in nn.Module
        num_ftrs = self.resnet18.fc.in_features
        self.resnet18.fc = torch.nn.Linear(num_ftrs, 1) # fc: variable in resnet class (which also looks like a class)

        # look at resnet class

    def forward(self, x):
        output = self.resnet18(x)
        output = torch.squeeze(output, dim = -1) # look at squeeze
        return output


# can easily write another model right here and then call that instead
